...
void UserDataTypeListener::on_data_available(DataReader* reader)
{
    ...
    for (i = 0; i < data_seq.length(); ++i) {
		...
		temp =test.encode(data_seq[i].ch);
        bool flag = strcmp(temp.c_str(),data_seq[i].md5);
        if(flag != 0)
        {
			cout<<"data error"<<endl;
			cout<<data_seq[i].ch<<endl;
			cout<<temp<<endl;
		}
  
		else
		{
			cout<<"UserType"<<endl<<data_seq[i].ch<<endl<<data_seq[i].md5<<endl;
        }
    }
